...
def test_fetch_swaps_mock(mock_post):
    ...
    swaps = fetch_swaps(
        "WETH/USDC", "0x8ad599c3a0ff1de082011efddc58f1908eb6e6d8", limit=1
    )
...
def test_uniswap_fetch_error(mock_post):
    mock_post.return_value.json.return_value = {"errors": [{"message": "Invalid"}]}
...
def test_uniswap_fetch_empty(mock_post):
    mock_post.return_value.json.return_value = {"data": {"swaps": []}}
